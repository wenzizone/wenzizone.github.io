...
这是一个 Terragrunt 系列的第一篇。后面几篇会结合我们自己维护的 OCI OKE 集群 Terraform 仓库，讲三层 `include` 结构、模块要不要独立成仓库、Stacks 新特性这些进阶话题；但在那之前，先花一篇讲清楚 Terragrunt 到底是什么、解决什么问题——这样后面几篇里的每一个设计选择，才有地方"挂"。
...
这个例子只有一个 unit，还没有体现出 Terragrunt 真正的价值——它是在"有很多个 unit、彼此有共享配置、有依赖关系"的时候才开始发挥作用的。下一篇会展示我们实际维护七八个 OKE 集群时用到的三层 `include` 结构，看看真实场景下这套东西是怎么落地的。
